...
def MakeList(Actor_list, AC_list, LO_list, LOid_array, Line_list, Arrow_list):  # excel出力用リストの生成
    ...
    ex_list = [['0Active', '1CardNo', '2Mode', '3Name', '4Message', '5Description', '6AddState-List' \
                   ...
                   , '12Delay', '13From', '14To', '15O_Cc', '16Attachment-List', '17Attachment-Name-List',
                '18Roles-List', '19DisplayCondition']]
    ST2Num_list = [];
    ST2Num_list2 = [];
    STNum = ''
    AT2Num_list = [];
    ...
    for i in range(1, acct + 1):
        ...
        if AC_list[i][6] != '':
            ex_list2.append(AC_list[i][6])  # 3Name、dataが空ならラベルをNameとする
        else:
            ex_list2.append(label)

        if AC_list[i][7] != '':
            ex_list2.append(AC_list[i][7])  # 4Message、dataが空ならラベルをMessageとする
        else:
            ex_list2.append(label)
        ...
        if k32 <= 0:
            tkinter.messagebox.showinfo('Design Error',
                                        'The actor of the ActionCard ' + AC_list[i][0] + ' is not obtained.')
            sys.exit(1)
        elif k32 <= 9:
            k32 = '0' + str(k32)
        else:
        ...
        if AC_list[i][10] != '':
            ex_list2.append(AC_list[i][10])  # 7AddState-Name-List 手動入力優先
        else:
            ex_list2.append(label)
        ...
        if AC_list[i][15] != '':
            ex_list2.append(AC_list[i][15])  # 12delay
        else:
            ex_list2.append('0')
        ...
        if AC_list[i][20] != '':
            ex_list2.append(AC_list[i][20])  # 17Attachment-Name-List 手動入力優先
        else:
            ex_list2.append(label)
        ...
        if ex_list2[2] == 'Manual':  # Mode='Manual'時のみ入力 *今はexcel出力なのでOFF
            if AC_list[i][21] != '':
                ex_list2.append(AC_list[i][21])  # 手動入力優先
            else:
                ex_list2.append(AC_list[i][2])  # 親アクタを自動入力
        else:
            ex_list2.append('')
    ...
    # ST_,AT_表記の変換
    c = myFunction.altvalue(ex_list, ST2Num_list, 6)  # addstate-list
    ex_list = c
    c = myFunction.altvalue(ex_list, ST2Num_list, 9)  # removestate-list
    ex_list = c
    c = myFunction.altvalue(ex_list, ST2Num_list, 10)  # TriggerCondition
    ex_list = c
    c = myFunction.altvalue(ex_list, AT2Num_list, 16)  # attachment-list
    ex_list = c
    c = myFunction.altvalue(ex_list, AT2Num_list, 19)  # DisplayCondition
    ex_list = c
