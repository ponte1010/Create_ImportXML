...
def PurseXML(root):
        ...
        if j == 'Actor':  # アクター（子スイムレーン）の取得
            for mxCell in Ob.iter('mxCell'):
                ...
                if 'swimlane' in k.get('style') == False:  # アクターがスイムレーンでなければエラー
                    ...
                    tkinter.messagebox.showinfo('design error',
                                                'The actor ' + Ob.get('id') + ' is an incorrect swimlane.')
    ...
    # アクションカードのデータをリストに格納
    AC_list = [['0id', '1label', '2parent', '3CardNo', '4ShapeType', '5Mode', '6Name', '7Message', '8Description',
                '9AddStateList' \
                   , '10AddStateNameList', '11AddStateTo', '12RemoveStateList', '13TrigerCondition', '14TrigerTimer' \
                                                                                                     '15Delay',
                '16From', '17To', '18Cc', '19AttachmentList', '20AttachmentNameList', '21RolesList',
                '22DisplayCondition']]
    acct = 0;
    a = []
    ACid_array = np.array([])
    for j in root.iter('object'):
        if j.get('A_ShapeType') == 'ActionCard':
            ...
            a.append(k)
            a.append(str(acct));
            a.append(j.get('A_ShapeType'));
            a.append(j.get('B_Mode'))
            a.append(j.get('C_Name'));
            a.append(j.get('D_Message'));
            a.append(j.get('E_Description'))
            a.append(j.get('F_AddStateList'));
            a.append(j.get('G_AddStateNameList'))
            a.append(j.get('H_AddStateTo'));
            a.append(j.get('I_RemoveStateList'))
            a.append(j.get('J_TrigerCondition'));
            a.append(j.get('K_TrigerTimer'))
            a.append(j.get('L_Delay'));
            a.append(j.get('M_From'))
            a.append(j.get('N_To'));
            a.append(j.get('O_Cc')), a.append(j.get('P_AttachmentList'))
            a.append(j.get('Q_AttachmentNameList'));
            a.append(j.get('R_RolesList'))
        ...
        if j != None:
            ...
            if dict.get('edgeStyle') == None:
                ...
                if index11 == False or index12 == False:  # ラインのターゲットがアクションカードまたは論理演算子でなければエラー
                    tkinter.messagebox.showinfo('Design Error',
                                                'Line ' + Ob.get('id') + ' does not have the correct target.')
                    ...
                    tkinter.messagebox.showinfo('Design Error',
                                                'Line ' + Ob.get('id') + ' does not have the correct source.')
                    print('Line ' + Ob.get('id') + ' does not have the correct source.')
                    sys.exit(1)
                else:
                    a.append(mxCell.get('id'));
                    a.append(mxCell.get('source'));
                    a.append(mxCell.get('target'))
                    Line_list.append(a);
                    a = []
                ...
                if index1 == False:  # アローのターゲットがアクションカードでなければエラー
                    tkinter.messagebox.showinfo('design error',
                                                'Arrow ' + Ob.get('id') + ' does not have the correct target.')
                    print('Arrow ' + Ob.get('id') + ' does not have the correct target.')
                    sys.exit(1)
                elif index2 == False:  # アローのソースがアクションカードでなければエラー
                    tkinter.messagebox.showinfo('design error',
                                                'Arrow ' + Ob.get('id') + ' does not have the correct source.')
    ...
    for i in LO_list[1:]:  # LO_listに入力情報を追加
        ...
        for j in range(len(k1[0])):
            ...
            if k2[0] >= 1:
                ...
                mae = mae + 'ST_' + Line_list[k1[0][j]][1] + ','  # 前がアクションカードなら、ST_カードのid
    ...
    while ct > 0:
        for i in range(len(skip_list)):  # スキップした要素を入力
            ...
            if k1.find('LO_') == -1:
                ...
                skip_list2.append(skip_list[i])
        skip_list = skip_list2;
        skip_list2 = []
