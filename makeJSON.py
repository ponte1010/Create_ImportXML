...
def SettingJson(TeamData_list):
    ...
    # setting.jsonの出力
    myFunction.JsonExport('setting.json', setting_dict)
...
def ContactsJson(TeamData_list, Actor_list):
    ...
    for i in range(2, len(Actor_list)):
        AcName = Actor_list[i][3];
        Role = Actor_list[i][4];
        RoleName = Actor_list[i][4];
        Email = AcName + '@' + TeamAddress;
        Team = TeamAddress;
        Desc = Actor_list[i][5]
        if Actor_list[i][2] == 'Auto':
            ...
            bool1 = False
        System = [bool1]
        k = {"name": AcName, "role": Role, "rolename": RoleName, "email": Email, "team": Team, "desc": Desc,
             "recipients": Recipients, "system": System}
    ...
    Email = ActName + '@' + TeamAddress;
    Team = TeamAddress
    bool1 = False;
    System = [bool1];
    Recipients = ["all"];
    Isadmin = True
    k = {"name": ActName, "role": Role, "rolename": RoleName, "email": Email, "team": Team, "system": System,
         "recipients": Recipients, "isadmin": Isadmin}
    contacts_list.append(k)
    contacts_dict = {"contacts": contacts_list}
    # contacts.jsonの出力
    myFunction.JsonExport('contacts.json', contacts_dict)
...
def ActionsJson(ex_list, Actor_list):
    ...
    for i in range(1, len(ex_list)):
        ...
        if ACactive == 0:  # 0で終了
            ...
            if ACMode == 'Manual':  # Manualなら
                ACName = ex_list[i][3];
                ACMessage = ex_list[i][4];
                ACDesc = ex_list[i][5]
                ACAttachments = [];
                ACRoles = [];
                ACSC = []
                ACAttachments.append(ex_list[i][16]);
                ACRoles.append(ex_list[i][18]);
                ACSC.append(ex_list[i][19])
                ...
                ACAttachments = []
                ACAttachments.append(ex_list[i][16]);
                ACRSC.append(ex_list[i][9]);
                ACSSC.append(ex_list[i][10])
    ...
    # 通知
    bool1 = True
    k = {"name": "通知", "type": "notification", "id": "s0000", "roles": ACRoles, "assignTo": ACRoles,
         "description": "システムからの通知メッセージです。", "hidden": bool1}
    actions_list.append(k)
    # 相談
    k = {"name": "相談", "type": "talk", "id": "i0001", "roles": ACRoles, "assignTo": ACRoles,
         "description": "任意のコミュニケーションに使用します。"}
    actions_list.append(k)
    # 報告・連絡
    k = {"name": "報告・連絡", "type": "action", "id": "i0009", "attach": bool1, "roles": ACRoles,
         "description": "他ロールへのインシデント情報の共有・展開"}
    actions_list.append(k)
    actions_dict = {"actions": actions_list}
    # actions.jsonの出力
    myFunction.JsonExport('actions.json', actions_dict)
...
def StatesJson(ex_list):
    states_list = []
    for i in range(1, len(ex_list)):
        SAddstateList = ex_list[i][6];
        SName = ex_list[i][3];
        SAttachmentList = ex_list[i][16];
        SAttachmentNameList = ex_list[i][17]
        # State
        if SAddstateList != "":
            k = {}
            k["id"] = SAddstateList
    ...
    states_list.append(k)
    # states.jsonの出力
    myFunction.JsonExport('states.json', states_list)
...
def RepliesJson(ex_list):
    ...
    # 自動入力
    bool1 = True
    k = {"actionid": "i0001", "to": "all", "name": "相談"};
    reply_list.append(k)
    k = {"actionid": "all", "to": "all", "name": "こちらの対応範囲外です。別のところにお問い合わせ下さい。", "delay": 5, "abort": bool1,
         "type": "notfound"};
    reply_list.append(k)
    k = {"actionid": "all", "to": "all", "name": "自動応答２", "type": "null", "delay": 0, "abort": bool1};
    reply_list.append(k)
    # message_list
    k = {"actionid": "8000", "text": "8000のテキスト"};
    ...
    # actions.jsonの出力
    myFunction.JsonExport('replies.json', replies_dict)
...
def PointsJson(ex_list):
    ...
    # points.jsonの出力
    myFunction.JsonExport('points.json', points_dict)
